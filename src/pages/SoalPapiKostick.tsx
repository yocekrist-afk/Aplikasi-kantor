...
export function SoalPapiKostick() {
  const { data: firestoreData, addData, updateData, deleteData, loading } = useFirestore('soal_papi_kostick');
  ...
  const [formData, setFormData] = useState({ id: '', subtest: 'Papi Kostick', soal: 'Pilih pernyataan yang paling menggambarkan diri Anda:', jawaban: '' });
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        soal: formData.soal || 'Pilih pernyataan yang paling menggambarkan diri Anda:',
        pilihan: choices,
        jawaban: formData.jawaban || '-',
        subtest: formData.subtest || 'Papi Kostick',
        jenisSoal: 'PG',
      ...
      console.error(e);
      alert('Gagal menyimpan soal Papi Kostick');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <>
      <DataTable 
        title="Soal Papi Kostick" columns={columns} data={data} 
        onAdd={() => { setFormData({ id: '', subtest: 'Papi Kostick', soal: 'Pilih pernyataan yang paling menggambarkan diri Anda:', jawaban: '' }); setChoices(['', '']); setIsModalOpen(true); }} addLabel="+ Add Soal" 
        onEdit={handleEdit} onDelete={handleDelete}
      />
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={!formData.id ? "Tambah Soal Papi Kostick" : "Edit Soal Papi Kostick"} maxWidth="max-w-2xl">
        <div className="space-y-6">
          ...
          <div>
            <label className="block text-sm text-gray-600 mb-2">Jawaban Benar (Papi Kostick tidak ada jawaban benar/salah)</label>
