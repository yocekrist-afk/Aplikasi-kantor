...
export function SoalGayaBelajar() {
  const { data: firestoreData, addData, updateData, deleteData, loading } = useFirestore('soal_gaya_belajar');
  ...
  const [formData, setFormData] = useState({ id: '', subtest: 'Gaya Belajar', soal: '', jawaban: '' });
        ...
        <div className="flex flex-col space-y-1.5 py-1">
          {choices && choices.map((choice, idx) => (
            <div key={idx} className="flex items-start space-x-2.5">
              <span className="w-5 h-5 rounded-full bg-slate-500 text-white flex items-center justify-center text-[11px] font-bold shrink-0 mt-0.5">{String.fromCharCode(65 + idx)}</span>
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        subtest: formData.subtest || 'Gaya Belajar',
        jenisSoal: 'PG',
      ...
      console.error(e);
      alert('Gagal menyimpan soal gaya belajar');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <>
      <DataTable 
        title="Soal Gaya Belajar" columns={columns} data={data} 
        onAdd={() => { setFormData({ id: '', subtest: 'Gaya Belajar', soal: '', jawaban: '' }); setChoices(['', '', '']); setIsModalOpen(true); }} addLabel="+ Add Soal" 
        onEdit={handleEdit} onDelete={handleDelete}
      />
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={!formData.id ? "Tambah Soal Gaya Belajar" : "Edit Soal Gaya Belajar"} maxWidth="max-w-2xl">
        <div className="space-y-6">
          <div>
            ...
            <select value={formData.subtest} onChange={e => setFormData({...formData, subtest: e.target.value})} className="w-full border border-gray-300 rounded-md px-4 py-2.5 text-sm focus:outline-none focus:border-[#8BC34A] bg-white">
              <option>Gaya Belajar</option>
            </select>
          </div>
          <div>
            <label className="block text-sm text-gray-600 mb-2">Pertanyaan</label>
            <textarea rows={3} value={formData.soal} onChange={e => setFormData({...formData, soal: e.target.value})} className="w-full border border-gray-300 rounded-md px-4 py-2.5 text-sm focus:outline-none focus:border-[#8BC34A]" />
          </div>
          <div>
            <label className="block text-sm text-gray-600 mb-2">Jawaban Benar (VAK tidak ada jawaban benar)</label>
          ...
          <div className="space-y-3">
            {choices.map((choice, idx) => (
              <div key={idx} className="flex items-center space-x-3">
                <span className="text-sm font-medium text-gray-700 w-5 text-center">{String.fromCharCode(65 + idx)}</span>
